  ...
  const handleButton = async (type, item) => {
    switch (type) {
      case 'chi-tiet':
        ...
        break;
      case 'danh-sach-grade':
        navigate(`${item?.id}/grades`, { state: { item } });
        break;
      case 'delete':
        var res = await requestDELETE(`api/v1/booktypes/${item.id}`);
        if (res) {
        ...
        return (
          <div className="d-flex">
            ...
            <a
              className="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 mb-1"
              data-toggle="m-tooltip"
              title="Xem danh sách khối lớp"
              onClick={() => {
                handleButton(`danh-sach-grade`, record);
              }}
            >
