...
const DoiSoatGiaoDichPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'fullName',
      key: 'fullName',
      render: (text, record, index) => <TDTableColumnHoTen showMenu={true} dataUser={{ type: 0, fullName: record?.fullName, imageUrl: record?.imageUrl, userName: record?.userName }} index={index} />,
    },
    {
      title: 'Trạng thái',
      dataIndex: 'status',
  ...
  const handleExport = async () => {
    try {
      ...
      const resExport = await requestDOWNLOADFILE(`api/v1/reports/export-doi-soat-giao-dich-all`, body);
    ...
    <>
      <div className="card card-xl-stretch mb-xl-9">
        <div className="p-4 border-bottom border-secondary border-bottom-solid d-flex align-items-center justify-content-between">
          <h3 className="card-title fw-bold text-header-td fs-4 mb-0">{'Thống kê giao dịch theo loại khoá học'}</h3>
        </div>
        <div className="p-5">
