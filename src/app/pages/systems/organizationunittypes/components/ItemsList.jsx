...
const UsersList = () => {
  ...
  const handleButton = async (type, item) => {
    switch (type) {
      ...
      case 'delete':
        var res = await requestDELETE(`api/v1/organizationunittypes/${item.id}`);
        if (res) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      render: (text, record) => {
        return <div className={`badge badge-light-${record?.isActive ? 'success' : 'danger'}`}>{`${record?.isActive ? 'Sử dụng' : 'Không sử dụng'}`}</div>;
      },
    },
    {
      title: 'Thao tác',
      ...
      render: (text, record) => {
        return (
          <div>
            <a
              className="btn btn-icon btn-light-success  btn-sm me-1 mb-1"
            ...
            <Popconfirm
              ...
              cancelText="Huỷ"
            >
              <a className="btn btn-icon btn-light-danger btn-sm me-1 mb-1" data-toggle="m-tooltip" title="Xoá">
