...
const ModalItem = props => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/popups/${id}`);

      var _data = res?.data ?? null;
      if (_data) {
        form.setFieldsValue({ ..._data });
        setImage(handleImage(_data?.source ?? '', FILE_URL));
      }
      setLoadding(false);
    };
    if (id) {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT_NEW(`api/v1/popups/${id}`, formData) : await requestPOST_NEW(`api/v1/popups`, formData);
          ...
          {!loadding && (
            <Form form={form} layout="vertical" autoComplete="off">
              <div className="row">
                ...
                <div className="col-xl-6 col-lg-6">
                  <FormItem label="Đường dẫn" name="link">
                    <Input placeholder="" />
                  </FormItem>
                </div>
