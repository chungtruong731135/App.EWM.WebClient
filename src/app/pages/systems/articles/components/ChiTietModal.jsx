...
const ModalItem = props => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (_data) {
        _data.publishingDate = _data?.publishingDate ? dayjs(_data.publishingDate) : null;
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT_NEW(`api/v1/articles/${id}`, formData) : await requestPOST_NEW(`api/v1/articles`, formData);
                  ...
                  <FormItem label="Chuyên mục" name="articlecatalog">
                    <TDSelect
                      ...
                      fetchOptions={async keyword => {
                        const res = await requestPOST(`api/v1/articlecatalogs/search`, {
                          pageNumber: 1,
                          pageSize: 1000,
                          advancedSearch: {
                            fields: ['name'],
                      ...
                      onChange={(value, current) => {
                        if (value) {
                          ...
                          form.setFieldValue('articlecatalogName', current?.name);
                        } else {
