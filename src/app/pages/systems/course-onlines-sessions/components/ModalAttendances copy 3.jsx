...
const EditableCell = ({ title, editable, children, dataIndex, record, editing, handleSave, ...restProps }) => {
  ...
  if (editable) {
    childNode = editing ? (
      ...
      >
        {dataIndex == 'status' ? <Checkbox onClick={save} /> : dataIndex == 'comment' ? <Input.TextArea ref={inputRef} onPressEnter={save} /> : <Checkbox onClick={save} />}
      </Form.Item>
    ) : (
      <div
...
const ModalItem = props => {
          ...
          <>
            <div className="d-flex align-items-center">
              ...
              <div className="d-flex flex-column " style={{ width: '150px' }}>
                <a href="#" className="text-gray-800 text-hover-primary mb-1 fw-bolder ellipsis">
                  {record?.fullName}
                </a>
                <span className="ellipsis">{record?.userName}</span>
              </div>
            </div>
          </>
        );
  ...
  const onFinishItem = async datas => {
    ...
    try {
      ...
      await requestPOST_NEW(`api/v1/classsessionattendances`, body);
    } catch (errorInfo) {
