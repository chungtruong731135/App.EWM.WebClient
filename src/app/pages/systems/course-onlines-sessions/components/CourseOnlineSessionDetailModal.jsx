...
import { Form, Input, Spin, InputNumber, DatePicker, Radio } from 'antd';
...
const ModalItem = props => {
  ...
  const modalVisible = useSelector(state => state.modal.courseOnlineClassSessionDetailModalVisible);
  ...
  const handleCancel = () => {
    form.resetFields();
    dispatch(actionsModal.setCourseOnlineClassSessionDetailModalVisible(false));
  };
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      body.gradeIds = body?.grades?.length > 0 ? body?.grades?.map(i => i.value)?.join('##') : null;
      ...
      // console.log(body);
      const res = id ? await requestPUT_NEW(`api/v1/classsessions/${id}`, body) : await requestPOST_NEW(`api/v1/classsessions`, body);
  ...
  return (
    <TDModal
      title={id ? 'Chi tiết buổi học' : 'Tạo mới buổi học'}
      show={modalVisible}
      fullscreen={'lg-down'}
      size={'xl'}
        ...
        {!loadding && (
          <Form form={form} layout="vertical" autoComplete="off" onFinishFailed={onFinishFailed} onFinish={onFinish}>
            <div className="row">
              ...
              <div className="col-xl-6 col-lg-6">
                <FormItem label="Tên buổi học" name="name" rules={[{ required: true, message: 'Không được để trống!' }]}>
                  <Input
                    placeholder=""
                    onChange={e => {
                      if (!id) {
              ...
              <div className="col-xl-12">
                <FormItem label="Video nội dung buổi học (Khi buổi học kết thúc)">
                  <Radio.Group className="mb-3" onChange={e => setTypeVideo(e.target.value)} value={typeVideo}>
