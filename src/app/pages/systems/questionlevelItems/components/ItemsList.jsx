...
const UsersList = props => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await requestPOST(
          `api/v1/questionlevelItems/search`,
          _.assign(
            {
              advancedSearch: {
                fields: ['name', 'code'],
  ...
  const handleButton = async (type, item) => {
    switch (type) {
      case 'chi-tiet':
        ...
        break;
      case 'delete':
        var res = await requestDELETE(`api/v1/questionlevelItems/${item.id}`);
        if (res) {
      ...
      case 'questions':
        navigate({
          pathname: `${item?.id}/questionOrders`,
        });
        break;
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'type',
      key: 'type',
      render: (text, record) => <div>{text ? DATA.find(i => i.value == text).label : ''}</div>,
    },
    {
      title: 'Nội dung',
      dataIndex: 'content',
      ...
      render: (text, record) => {
        return (
          <div className="d-flex">
            ...
            {record?.type == 'Exercise' || record?.type == 'Quiz' || record?.type == 'UnitTest' ? (
              <a
                className="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 mb-1"
                data-toggle="m-tooltip"
                title="Câu hỏi"
