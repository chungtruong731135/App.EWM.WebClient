...
const UsersList = props => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await requestPOST(
          `api/v1/learningroadmaps/search`,
          _.assign(
            {
              advancedSearch: {
                fields: ['title', 'code'],
  ...
  const handleButton = async (type, item) => {
    switch (type) {
      ...
      case 'items':
        navigate({
          pathname: `${item?.id}/roadmapitems`,
        });

        break;
      case 'delete':
        var res = await requestDELETE(`api/v1/learningroadmaps/${item.id}`);
        if (res) {
        ...
        return (
          <div className="d-flex ">
            ...
            <a
              className="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 mb-1"
              data-toggle="m-tooltip"
              title="Danh sách các gợi ý học"
              onClick={() => {
                handleButton(`items`, record);
              }}
            >
