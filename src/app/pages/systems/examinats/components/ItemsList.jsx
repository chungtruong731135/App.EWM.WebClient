...
const UsersList = props => {
  ...
  const handleButton = async (type, item) => {
    switch (type) {
      ...
      case 'delete':
        var res = await requestDELETE(`api/v1/examinats/${item.id}`);
        if (res) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      render: (text, record) => {
        return <div className={`badge badge-light-${record?.published ? 'success' : 'danger'}`}>{`${record?.published ? 'Phát hành' : 'Không phát hành'}`}</div>;
      },
    },
    {
      title: 'Thao tác',
