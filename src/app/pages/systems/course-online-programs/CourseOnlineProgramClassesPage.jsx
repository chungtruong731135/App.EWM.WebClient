...
const UsersPage = props => {
  ...
  const { courseOnlineProgramId } = useParams();

  const [dataSearch, setDataSearch] = useState({ courseOnlineProgramId: courseOnlineProgramId });
  ...
  useEffect(() => {
    ...
    if (courseOnlineProgramId) {
      fetchData(courseOnlineProgramId);
    }
  }, [courseOnlineProgramId]);
      ...
      <div className="card card-xl-stretch mb-xl-3">
        <div className="px-3 py-3 d-flex align-items-center justify-content-between">
          <div className="d-flex align-items-center">
            ...
            <h3 className="card-title fw-bolder text-header-td fs-2 mb-0">{'Danh sách lớp học của chương trình tuyển sinh ' + (dataCourse?.name ?? '')}</h3>
          </div>
        </div>
      </div>
                ...
                onClick={() => {
                  dispatch(
                    actionsModal.setCourseOnlineClassDetail({
                      ...null,
                      courseOnlineProgramId: dataCourse?.id,
                      courseOnlineProgramName: dataCourse?.name,
                    })
                  );
