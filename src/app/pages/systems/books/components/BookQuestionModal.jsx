...
const ModalItem = props => {
  ...
  // console.log('table', props?.bookTable)
    ...
    const fetchData = async () => {
      ...
      if (id) {
        ...
        form.setFieldsValue({
          ...
          bookTypeName,
          bookSubjectName,
          // bookTableName: parentItem ? `${parentItem?.bookTableName} (${parentItem?.bookTableParentName ?? ''})` : null,
          ...
          bookTableId: props?.bookTable?.id ?? null,
          bookTableName: props?.bookTable
            ? `${props?.bookTable?.name}${props?.bookTable?.parentAndGrandParentName ? ` (${props?.bookTable?.parentAndGrandParentName})` : ''}`
            : null,
        });
      }
  ...
  }, [id, props?.bookItem, props?.bookTable, form, questionType]);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData = {
        ...
        // Gán bookTableId theo thằng bố nó nếu có parentItem, hoặc nếu có bookTable từ props, dùng nó khi thêm mới
        bookTableId: parentItem?.bookTableId ?? values.bookTableId ?? props?.bookTable?.id,
      };
          ...
          {!loadding && (
            <Form form={form} layout="vertical" autoComplete="off" onFinish={onFinish} onFinishFailed={onFinishFailed}>
              <div className="row">
                ...
                <div className="col-xl-6 col-lg-6">
                  <Form.Item label="Sách" name="bookName" rules={[{ required: true, message: 'Không được để trống!' }]}>
                    <TDSelect
                      disabled
                      reload
                      showSearch
                ...
                <div className="col-xl-6 col-lg-6">
                  <Form.Item label="Thuộc danh mục bộ sách" name="bookTypeName" /*rules={[{ required: true, message: 'Không được để trống!' }]}*/>
                    <TDSelect
                      disabled
                      reload
                      showSearch
                ...
                <div className="col-xl-6 col-lg-6">
                  <Form.Item label="Thuộc danh mục môn học" name="bookSubjectName" /*rules={[{ required: true, message: 'Không được để trống!' }]}*/>
                    <TDSelect
                      disabled
                      reload
                      showSearch
                ...
                <div className="col-xl-6 col-lg-6">
                  ...
                  >
                    <TreeSelect
                      showSearch
                      style={{ width: '100%' }}
                      value={form.getFieldValue('bookTableId')}
                      ...
                      treeData={bookTables}
                      disabled={!!parentItem || !!parentId || bookTables?.length === 0} // dis thêm nếu có parentItem
                    />
                  </Form.Item>
                </div>
