...
const ModalItem = props => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/booktables/${id}`);

      var _data = res?.data ?? null;
      if (_data) {
        _data.publishDate = _data?.publishDate ? dayjs(_data.publishDate) : null;
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT_NEW(`api/v1/booktables/${id}`, formData) : await requestPOST_NEW(`api/v1/booktables`, formData);
      ...
      if (res.status === 200) {
        ...
        const errors = Object.values(res?.data?.messages[0] ?? {});
        let final_arr = [];
        errors.forEach(item => {
  ...
  useEffect(() => {
    if (bookTable) form.setFieldsValue({ parentName: bookTable.name, parentId: bookTable.id });
  }, []);
            ...
            <Form form={form} layout="vertical" /* initialValues={initData} */ autoComplete="off">
              <div className="row">
                <div className="col-xl-6 col-lg-6">
                  <FormItem label="Tên mục lục" name="name" rules={[{ required: true, message: 'Không được để trống!' }]}>
                    <Input
                      placeholder=""
                      onChange={e => {
                        if (!id) {
                ...
                <div className="col-xl-6 col-lg-6">
                  <FormItem label="Thuộc mục lục" name="parentName">
                    <TDSelect
                      disabled={id}
                      placeholder=""
                ...
                <div className="col-xl-6 col-lg-6">
                  <FormItem label="Mức độ ưu tiên" name="pageOrder">
