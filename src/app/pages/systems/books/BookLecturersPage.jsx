...
import BookLectureModal from './components/BookLectureModal';
import clsx from 'clsx';
import { toast } from 'react-toastify';

const BookLecturersPage = () => {
  ...
  const [bookLecture, setBookLecture] = useState(null);
  useEffect(() => {
        ...
        if (res.data.length > 0) {
          const groupedData = res.data.reduce((acc, item) => {
            if (!item.parentId) {
              acc.push({
                ...item,
                children: res.data.filter(child => child.parentId === item.id).length > 0 ? res.data.filter(child => child.parentId === item.id).sort((a, b) => a.pageOrder - b.pageOrder) : null,
              });
            }
            return acc;
          }, []);
          setDataTables(groupedData.sort((a, b) => a.pageOrder - b.pageOrder) ?? []);
          ...
          <div>
            ...
            {!record?.parentId && (
              <a
                className="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 mb-1"
                data-toggle="m-tooltip"
                title="Thêm bài giảng"
                onClick={() => {
                  handleButton(`them-bai-giang`, record);
                }}
              >
  ...
  const handleButton = async (type, item) => {
    switch (type) {
      case 'chi-tiet':
        setBookLecture({ id: item.id, name: item.name });
        dispatch(actionsModal.setDataModal(item));
        dispatch(actionsModal.setModalVisible(true));
        break;
      case 'delete':
        var res = await requestDELETE(`api/v1/booklectures/${item.id}`);
        if (res) {
  ...
  return (
    <>
      <div className="card card-xl-stretch mb-xl-3">
        <div className="px-3 py-3 d-flex align-items-center justify-content-between">
          <div className="d-flex align-items-center">
            ...
            <h3 className="card-title fw-bolder text-header-td fs-2 mb-0">{`Danh sách bài giảng`}</h3>
          </div>
      ...
      {modalVisible ? <BookLectureModal bookLecture={bookLecture} setBookLecture={setBookLecture} /> : <></>}
    </>
  );
};

export default BookLecturersPage;
