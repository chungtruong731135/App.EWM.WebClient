...
const ModalItem = props => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/dictionarywords/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT_NEW(`api/v1/dictionarywords/${id}`, formData) : await requestPOST_NEW(`api/v1/dictionarywords`, formData);
          ...
          {!loadding && (
            <Form form={form} layout="vertical" autoComplete="off">
              <div className="row">
                ...
                <div className="col-xl-6 col-lg-6">
                  <FormItem label="Tên tiếng Anh" name="title" rules={[{ required: true, message: 'Không được để trống!' }]}>
                    <Input placeholder="" />
                  </FormItem>
                </div>
                <div className="col-xl-6 col-lg-6">
                  <FormItem label="Nghĩa tiếng Việt" name="vnMeaning" rules={[{ required: true, message: 'Không được để trống!' }]}>
                    <Input placeholder="" />
                  </FormItem>
                </div>
                <div className="col-xl-6 col-lg-6">
